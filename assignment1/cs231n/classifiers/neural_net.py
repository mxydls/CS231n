...
class TwoLayerNet(object):
	...
	def loss(self, XT, y=None, reg=0.0):
		"""
		Compute the loss and gradients for a two layer fully connected neural
		network.

		Inputs:
		- XT: Input data of shape (N, D). Each XT[i] is a training sample.
		- y: Vector of training labels. y[i] is the label for XT[i], and each y[i] is
		...
		the score for class c on input XT[i].
		...
		RW1, RW2 = W1 ** 2, W2 ** 2
		loss = LT.sum() + reg * RW1.sum() + reg * RW2.sum()
	...
	def train(self, XT, y, X_val, y_val,
		...
		Inputs:
		- XT: A numpy array of shape (N, D) giving training data.
		- y: A numpy array f shape (N,) giving training labels; y[i] = c means that
			XT[i] has label c, where 0 <= c < C.
	...
	def predict(self, XT):
		...
		Inputs:
		- XT: A numpy array of shape (N, D) giving N D-dimensional data points to
			classify.

		Returns:
		- y_pred: A numpy array of shape (N,) giving predicted labels for each of
			the elements of XT. For all i, y_pred[i] = c means that XT[i] is predicted

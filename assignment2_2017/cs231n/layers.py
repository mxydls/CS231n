...
def batchnorm_forward(x, gamma, beta, bn_param):
	...
	if mode == 'train':
		...
		# At each iteration on batches, theses two things will update
		# They are the whole dataset's feature mean / var
		running_mean = momentum * running_mean + (1 - momentum) * sample_mean    
		running_var = momentum * running_var + (1 - momentum) * sample_var
		#############################################################################
		#                             END OF YOUR CODE                              #
	...
	# Store the updated running means back into bn_param directly
...
#Convolutional Layer A naive way
def conv_forward_naive(x, w, b, conv_param):
	...
	assert((H + 2 * pad - HH) % stride == 0 and (W + 2 * pad - WW) % stride == 0)
...
def max_pool_forward_naive(x, pool_param):
	...
	assert((H + pool_height) % stride == 0 and (W - pool_height) % stride == 0)
	...
	for i in range(N):       # ith image
		for f in range(C):   # fth filter
			for j in range(H_new):
				for k in range(W_new):
					window = x[i, f, j*stride:pool_height+j*stride, k*stride:pool_width+k*stride]
					out[i, f, j, k] = np.max(window)
	#############################################################################
	#                             END OF YOUR CODE                              #
	#############################################################################
	cache = (x, pool_param)
		...
		for c in xrange(C):
			for h in xrange(H_new):
				for wi in xrange(W_new):
					t = x[n, c, h*stride:h*stride+pool_height, wi*stride:wi*stride+pool_width]
					X, Y = np.where(t == np.amax(t))
					dx[n, c, h*stride+X[0], wi*stride+Y[0]] = dout[n, c, h, wi]
